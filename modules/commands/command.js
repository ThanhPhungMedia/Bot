...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  if (type == 'menu') {
    if (["01", "1", "02", "2"].includes(args[0])) {
      reply(`𝐇𝐚̃𝐲 𝐩𝐡𝐚̉𝐧 𝐡𝐨̂̀𝐢 𝐭𝐢𝐧 𝐧𝐡𝐚̆́𝐧 𝐧𝐚̀𝐲 𝐯𝐨̛́𝐢 ${["01", "1"].includes(args[0]) ? "𝐁𝐢𝐨" : "𝐁𝐢𝐞̣̂𝐭 𝐝𝐚𝐧𝐡"} 𝐁𝐚̣𝐧 𝐦𝐮𝐨̂́𝐧 đ𝐨̂̉𝐢 𝐜𝐡𝐨 𝐛𝐨𝐭 𝐡𝐨𝐚̣̆𝐜 '𝐃𝐞𝐥𝐞𝐭𝐞' 𝐍𝐞̂́𝐮 𝐦𝐮𝐨̂́𝐧 𝐱𝐨𝐚́ ${["01", "1"].includes(args[0]) ? "𝐁𝐢𝐨" : "𝐁𝐢𝐞̣̂𝐭 𝐝𝐚𝐧𝐡"} 𝐇𝐢𝐞̣̂𝐧 𝐭𝐚̣𝐢`, (err, info) => {
      ...
      const msg = messagePending.reduce((a, b) => a += `» ${b.name} | ${b.threadID} | 𝐓𝐢𝐧 𝐧𝐡𝐚̆́𝐧: ${b.snippet}\n`, "");
      return reply(`𝐃𝐚𝐧𝐡 𝐬𝐚́𝐜𝐡 𝐭𝐢𝐧 𝐧𝐡𝐚̆́𝐧 𝐜𝐡𝐨̛̀ 𝐜𝐮̉𝐚 𝐛𝐨𝐭:\n\n${msg}`);
    }
    else if (["04", "4"].includes(args[0])) {
      const messagePending = await api.getThreadList(500, null, ["unread"]);
      const msg = messagePending.reduce((a, b) => a += `» ${b.name} | ${b.threadID} | 𝐓𝐢𝐧 𝐧𝐡𝐚̆́𝐧: ${b.snippet}\n`, "") || "𝐊𝐡𝐨̂𝐧𝐠 𝐜𝐨́ 𝐭𝐢𝐧 𝐧𝐡𝐚̆́𝐧 𝐧𝐚̀𝐨";
      return reply(`𝐃𝐚𝐧𝐡 𝐬𝐚́𝐜𝐡 𝐭𝐢𝐧 𝐧𝐡𝐚̆́𝐧 𝐜𝐡𝐮̛𝐚 đ𝐨̣𝐜 𝐜𝐮̉𝐚 𝐛𝐨𝐭:\n\n${msg}`);
    }
    else if (["05", "5"].includes(args[0])) {
      const messagePending = await api.getThreadList(500, null, ["OTHER"]);
      const msg = messagePending.reduce((a, b) => a += `» ${b.name} | ${b.threadID} | 𝐓𝐢𝐧 𝐧𝐡𝐚̆́𝐧: ${b.snippet}\n`, "") || "𝐊𝐡𝐨̂𝐧𝐠 𝐜𝐨́ 𝐭𝐢𝐧 𝐧𝐡𝐚̆́𝐧 𝐧𝐚̀𝐨";
      return reply(`𝐃𝐚𝐧𝐡 𝐬𝐚́𝐜𝐡 𝐭𝐢𝐧 𝐧𝐡𝐚̆́𝐧 𝐬𝐩𝐚𝐦 𝐜𝐮̉𝐚 𝐛𝐨𝐭 :\n\n${msg}`);
    }

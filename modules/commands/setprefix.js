...
module.exports.handleReaction = async function({ api, event, multiple, Threads, handleReaction }) {
	...
	return api.sendMessage(`[𝘽𝙊𝙏 𝙍𝙮𝙤 🎀] »Đ𝐚̃ 𝐜𝐡𝐮𝐲𝐞̂̉𝐧 đ𝐨̂̉𝐢 𝐩𝐫𝐞𝐟𝐢𝐱 𝐜𝐮̉𝐚 𝐧𝐡𝐨́𝐦 𝐭𝐡𝐚̀𝐧𝐡: ${handleReaction.PREFIX}`, threadID);
}
...
module.exports.run = async ({ api, event, args, client, Threads }) => {
	...
	if (prefix == "reset") {
		...
		return api.sendMessage(`[𝘽𝙊𝙏 𝙍𝙮𝙤 🎀] »Đ𝐚̃ 𝐫𝐞𝐬𝐞𝐭 𝐩𝐫𝐞𝐟𝐢𝐱 𝐯𝐞̂̀ 𝐦𝐚̣̆𝐜 đ𝐢̣𝐧𝐡 ${global.config.PREFIX}`, event.threadID, event.messageID);
	} else return api.sendMessage("[𝘽𝙊𝙏 𝙍𝙮𝙤🎀] »𝐁𝐚̣𝐧 𝐜𝐨́ 𝐜𝐡𝐚̆́𝐜 𝐦𝐮𝐨̂́𝐧 đ𝐨̂̉𝐢 𝐩𝐫𝐞𝐟𝐢𝐱 𝐜𝐮̉𝐚 𝐧𝐡𝐨́𝐦 𝐭𝐡𝐚̀𝐧𝐡: " + prefix + "\n\n𝐓𝐡𝐚̉ 𝐜𝐚̉𝐦 𝐱𝐮́𝐜 𝐯𝐚̀𝐨 𝐭𝐢𝐧 𝐧𝐡𝐚̆́𝐧 đ𝐞̂̉ 𝐱𝐚́𝐜 𝐧𝐡𝐚̣̂𝐧.", event.threadID, (error, info) => {

...
module.exports.run = async function ({ api, Users, Threads, event, args }) {
  ...
  if (!args[0]) return api.sendMessage(`${api.getCurrentUserID()}`, () =>
    api.sendMessage(`[𝘽𝙊𝙏 𝙍𝙮𝙤 🎀] »★★ 𝗧𝗮̣𝗺 𝗕𝗶𝗲̣̂𝘁 𝗡𝗵𝗲́ ★★\n\n𝗼𝘂𝘁 𝗯𝗼𝘅 𝗱𝗮̂𝘆😢 `, event.threadID, () =>
      api.removeUserFromGroup(`${api.getCurrentUserID()}`, event.threadID)));
        api.sendMessage("[𝘽𝙊𝙏 𝙍𝙮𝙤 🎀] »𝗗𝗮̃ 𝗻𝗵𝗮̣̂𝗻 𝗹𝗲̣̂𝗻𝗵 𝗼𝘂𝘁 𝗻𝗵𝗼́𝗺 𝘁𝘂̛̀ 𝗮𝗱𝗺𝗶𝗻, 𝗹𝘆́ 𝗱𝗼: " + reason.join(" "), idbox, () =>
          api.removeUserFromGroup(`${api.getCurrentUserID()}`, idbox, () =>
            api.sendMessage("[𝘽𝙊𝙏 𝙍𝙮𝙤 🎀] »𝗗𝗮̃ 𝗼𝘂𝘁 𝗯𝗼𝘅 𝗰𝗼́ 𝗶𝗱: " + idbox + " với lý do: " + reason.join(" "), event.threadID)))
}